...
testY=Y(11:17,:);
%row是矩阵的行数，表示总共有多少个训练集。column是矩阵的列数，表示训练集的输入。
[row,column]=size(trainingX);
OutputLayerNum=1;%输出层神经元个数
v=rand(column,column+1);%随机初始化输入层与隐层的权值,v是一个column行column+1列矩阵
w=rand(column+1,OutputLayerNum);%随机初始化隐层与输出层的权值,w是一个column+1行向量

gamma=rand(column+1);%隐层阈值,gamma是column+1行向量
theta=rand(OutputLayerNum);%输出层阈值,theta是1行1列矩阵

output=zeros(row,OutputLayerNum);%输出层输出
hiddenOutput=zeros(column+1);%隐层输出
g=zeros(OutputLayerNum);%均方误差对w,gamma求导的参数
e=zeros(column+1);%均方误差对v,theta求导的参数
LearningRate=0.01;%学习率，在0-1之间
...
while(1)
    ...
    for i=1:row
      ...
      for j=1:column+1
        ...
        hiddenOutput(i,j)=1/(1+exp(-alpha+gamma(j)));%代入sigmoid函数
      end
       %计算输出层输出
       for j=1:OutputLayerNum
         beta=0;
         for k=1:column+1
           beta=beta+w(k,j)*hiddenOutput(i,k);
         end
    ...
    for i=1:row
        ...
        for j=1:column+1
          ...
          for k=1:OutputLayerNum
            ...
            e(j)=teh*hiddenOutput(i,j)*(1-hiddenOutput(i,j));%导数
        end
        %计算权值系数w、delta导数
        for j=1:OutputLayerNum
          delta_theta=delta_theta+(-1)*LearningRate*g(j);
          for k=1:column+1
            delta_w(k,j)=delta_w(k,j)+LearningRate*g(j)*hiddenOutput(i,k);
          end
        end
        %计算v、gamma导数
        for j=1:column+1
    ...
    %设置迭代终止条件：前后两次误差之差绝对值小于0.01%，且累计100次
    if(abs(previous_E-E)<0.0001)
      AccumulateTimes=AccumulateTimes+1;
      if(AccumulateTimes==100)%误差位于设定范围内累计100次
        break;
      end
    else
      previous_E=E;
...
for i=1:7
  for j=1:OutputLayerNum
    testE=testE+((testY(i)-testoutput(i,j))^2);
  end
end
%输出测试集均方误差sum
sum = testE/7
